...
| W02 | **8 / 8개 API 구현 완료**: 17·23·28·29·30·31·32·33 | DB 적용·최초 신청 접수 연결·MySQL 동시성·실연동 대기 | **부분 완료** |
...
| W01 / 06 FCM | 자신의 최신 토큰·갱신 시각 저장 | **완료** | 실제 푸시 발송은 W09 |
...
| W02 / 29 하위조직 생성 | 대표 전용·부모 범위·계약 날짜·조직 코드·정규 테이블·렌탈 지점 행 | **완료** | MySQL 동시 생성 검증 대기 |
...
| W02 / 32 신청 조회 | 대표의 현재 지점 REQUESTED 조회, 기존 admins(email/name) 응답 | **완료** | 최초 신청 접수·계정 생성 경로 연결 및 실제 DB 적용 대기 |
| W02 / 33 신청 승인/거절 | 신청 잠금·계정 재검증·일반 role/소속/scope·처리자/시각 일괄 반영 | **완료** | 중복·타 지점·비활성 계정·롤백 로컬 검증 완료. MySQL 동시성·접수·앱 연동 대기 |

15개 경로의 기본 기능 코드와 로컬 검증을 완료했다. 05·23도 승인된 저장 컬럼 기준으로 검증했다. 32·33은 저장된 신청을 조회·처리하며, 신청 접수 자체를 새로 제공하는 API는 아니다. DB 설정이 없으면 `DATABASE_NOT_CONFIGURED`, 마이그레이션 미적용·키 미설정 시 저장 오류가 발생하므로 실제 운영 가능을 뜻하지 않는다.
...
| 로컬 자동 테스트·명세 대조 | **완료** | 140개 테스트 통과, 15개 API의 Method·응답 예시와 MD·엑셀 대조 통과 |
...
- **남은 연결**: 현재 38개 명세에는 신규 계정 생성·신청 접수 API가 없다. 가입 서버에서 본인 확인·이메일 소유권·지점 검증 후 기존 MSP_ADMIN 계정과 신청을 연결해야 한다. 이번에는 새 접수 URL, 실제 신청/계정 데이터나 초대 코드 소비를 추가하지 않았다.
...
- **완료 (로컬 자동 테스트)**: 2026-09-18 재검증, `python -m pytest -q` 기준 **140 passed, 6 skipped**, SQLite·대역 기반 검증 통과. 관리자 3세션 상한·고객 access/refresh token 회전·로그아웃 폐기, 관리자·고객 refresh token 분리, 앱 FCM 토큰 저장·로그아웃 보존, 프로필 코드 검증과 기존 phone·샵 저장을 포함한다. FCM 알림 발송·재시도는 새 Firebase 프로젝트 연결 후 별도 구현한다.
