...
def login(client, uid):
    response = client.post("/api/v1/nadri/user/login", json={"UID": uid})
...
def test_nadri_app_fcm_token_is_stored_and_preserved_on_logout(setup):
    tables = rental_tables(setup)
    response = setup["client"].post("/api/v1/nadri/user/login", json={
    ...
    response = setup["client"].patch("/api/v1/nadri/user/profile", headers=headers,
                                      json={"fcmToken": "customer-device-token-2"})
    assert response.status_code == 200, response.text
    assert setup["client"].post("/api/v1/nadri/user/logout", headers=headers).status_code == 200
    with setup["engine"].connect() as connection:
        row = connection.execute(select(tables["MSP_RENTAL_USER"])).mappings().one()
...
def test_nadri_refresh_rotates_and_rejects_the_previous_token(setup):
    ...
    response = setup["client"].post("/api/v1/nadri/user/refresh", headers={"X-Refresh-Token": old_refresh})
    assert response.status_code == 200, response.text
    new_refresh = response.json()["refreshToken"]
    assert new_refresh != old_refresh
    assert setup["client"].post("/api/v1/nadri/user/refresh",
...
def test_nadri_login_keeps_one_active_refresh_token_per_uid(setup):
    ...
    assert setup["client"].post("/api/v1/nadri/user/refresh", headers={
...
def test_nadri_logout_revokes_access_and_refresh_tokens(setup):
    ...
    response = setup["client"].post("/api/v1/nadri/user/logout", headers=headers)
    assert response.status_code == 200, response.text
    assert setup["client"].post("/api/v1/nadri/user/refresh", headers={
        "X-Refresh-Token": headers["X-Refresh-Token"]}).status_code == 401
    assert setup["client"].patch("/api/v1/nadri/user/profile", headers={
...
def test_nadri_logout_keeps_access_only_compatibility(setup):
    ...
    response = setup["client"].post("/api/v1/nadri/user/logout", headers={
